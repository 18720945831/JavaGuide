...
Java虚拟机所管理的内存区域除程序计数器以外，都有发生OutOfMemory(OOM)异常的可能。常见的如Java堆异常，可能的情况为内存泄露（Memory Leak）、内存溢出（Memory Overflow）。如果是内存泄露，通过工具查看泄露对象到GC Roots的引用链，找出泄露对象和GC Roots的关联路径，查看垃圾收集器无法收集垃圾的原因。
